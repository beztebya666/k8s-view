...
export function buildSeed() {
  ...
  R["core/v1/pods"] = pods();
  R["apps/v1/deployments"] = [deployment("web", "default", 3, 3, 3 * D), deployment("api", "default", 2, 2, 2 * D), deployment("grafana", "monitoring", 1, 1, 8 * D), deployment("argocd-server", "argocd", 1, 1, 15 * D), deployment("cert-manager", "cert-manager", 1, 0, 10 * D), deployment("ingress-nginx-controller", "ingress-nginx", 2, 2, 9 * D)];
  R["apps/v1/replicasets"] = [["web-7d9f8c6b5", "default", 3], ["api-5c8b9d7f6", "default", 2]].map(([n, ns, r]: any) => ({ apiVersion: "apps/v1", kind: "ReplicaSet", metadata: meta(n, ns, 3 * D, { app: String(n).split("-")[0] }, { ownerReferences: [{ kind: "Deployment", name: String(n).split("-")[0] }] }), spec: { replicas: r }, status: { replicas: r, readyReplicas: r } }));
  R["apps/v1/statefulsets"] = [["postgres", "default", 1], ["prometheus", "monitoring", 1]].map(([n, ns, r]: any) => ({ apiVersion: "apps/v1", kind: "StatefulSet", metadata: meta(n, ns, 12 * D, { app: n }), spec: { replicas: r, serviceName: n }, status: { replicas: r, readyReplicas: r } }));
  R["apps/v1/daemonsets"] = [["kube-proxy", "kube-system"], ["node-exporter", "monitoring"]].map(([n, ns]: any) => ({ apiVersion: "apps/v1", kind: "DaemonSet", metadata: meta(n, ns, 20 * D, { app: n }), spec: {}, status: { desiredNumberScheduled: 3, numberReady: 3, numberAvailable: 3 } }));
  R["core/v1/services"] = [service("kubernetes", "default", "ClusterIP", "10.96.0.1", 443, 30 * D), service("web", "default", "ClusterIP", "10.96.42.10", 80, 3 * D), service("api", "default", "ClusterIP", "10.96.42.20", 8080, 2 * D), service("postgres", "default", "ClusterIP", "10.96.42.30", 5432, 12 * D), service("redis", "default", "ClusterIP", "10.96.42.40", 6379, 12 * D), service("grafana", "monitoring", "ClusterIP", "10.96.50.10", 3000, 8 * D), service("prometheus", "monitoring", "ClusterIP", "10.96.50.20", 9090, 8 * D), service("ingress-nginx", "ingress-nginx", "LoadBalancer", "10.96.60.10", 443, 9 * D)];
  R["core/v1/nodes"] = [node("node-1", "10.0.1.11", true, "cp"), node("node-2", "10.0.1.12", true, ""), node("node-3", "10.0.1.13", true, "")];
  R["core/v1/namespaces"] = NS.concat(["kube-public", "kube-node-lease"]).map((n) => ({ apiVersion: "v1", kind: "Namespace", metadata: meta(n, undefined, 30 * D), spec: {}, status: { phase: "Active" } }));
  R["core/v1/configmaps"] = [["coredns", "kube-system"], ["kube-root-ca.crt", "default"], ["app-config", "default"], ["grafana-dashboards", "monitoring"]].map(([n, ns]: any) => ({ apiVersion: "v1", kind: "ConfigMap", metadata: meta(n, ns, 10 * D), data: { "config.yaml": "key: value\nlog_level: info" } }));
  R["core/v1/secrets"] = [["db-credentials", "default", "Opaque"], ["tls-cert", "ingress-nginx", "kubernetes.io/tls"], ["argocd-secret", "argocd", "Opaque"]].map(([n, ns, t]: any) => ({ apiVersion: "v1", kind: "Secret", metadata: meta(n, ns, 10 * D), type: t, data: { token: "••••••••" } }));
  R["batch/v1/cronjobs"] = [["backup", "default"], ["cert-renew", "cert-manager"]].map(([n, ns]: any) => ({ apiVersion: "batch/v1", kind: "CronJob", metadata: meta(n, ns, 9 * D), spec: { schedule: "0 2 * * *", suspend: false }, status: { lastScheduleTime: ts(20 * H) } }));
  R["batch/v1/jobs"] = [["migrate-27839", "default", true], ["broken-job", "default", false]].map(([n, ns, ok]: any) => ({ apiVersion: "batch/v1", kind: "Job", metadata: meta(n, ns, 4 * H), spec: { completions: 1 }, status: ok ? { succeeded: 1, completionTime: ts(3 * H) } : { failed: 8, conditions: [{ type: "Failed", status: "True", reason: "BackoffLimitExceeded" }] } }));
  R["networking.k8s.io/v1/ingresses"] = [["web", "default", "app.acme.io"], ["grafana", "monitoring", "grafana.acme.io"]].map(([n, ns, host]: any) => ({ apiVersion: "networking.k8s.io/v1", kind: "Ingress", metadata: meta(n, ns, 9 * D), spec: { rules: [{ host, http: { paths: [{ path: "/", backend: { service: { name: n, port: { number: 80 } } } }] } }] }, status: { loadBalancer: { ingress: [{ ip: "10.96.60.10" }] } } }));
  R["core/v1/persistentvolumeclaims"] = [["postgres-data", "default", "20Gi"], ["prometheus-data", "monitoring", "50Gi"]].map(([n, ns, size]: any) => ({ apiVersion: "v1", kind: "PersistentVolumeClaim", metadata: meta(n, ns, 12 * D), spec: { accessModes: ["ReadWriteOnce"], resources: { requests: { storage: size } }, storageClassName: "standard" }, status: { phase: "Bound", capacity: { storage: size } } }));
  R["core/v1/events"] = [
